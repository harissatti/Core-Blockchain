...
contract Al_Hejin_CamelNFT is ERC721A,ReentrancyGuard,AccessControl{
    ...
    constructor(address _tokenAddress,string memory baseUri,uint _price,uint _Max_Supply,uint256 _reserveQuantity) ERC721A("Al-Hejin","ALH", _reserveQuantity) {
         baseURI= baseUri;
    ...
    function setMaxSupply(uint256 _maxSupply) public onlyRole(ADMIN_ROLE) {
        require(Max_Supply < _maxSupply, "Max supply should be greater then previous");
    ...
    function reserve(uint256 quantity, address to) public onlyRole(ADMIN_ROLE) {
        require(quantity <= reserveQuantity, "the quantity exceeds reserve");
        require(_nextReserveTokenId() + quantity - 1 <= _startTokenId() - 1, "Not enough tokens left");
    ...
    function adminMint(uint256 pack,address to) public onlyRole(MINTER_ROLE){
         ...
         require(_nextTokenId() + nftIdsToMint - 1 <= Max_Supply, "Not enough tokens left");
         _safeMint(to, nftIdsToMint, false);
    }
    ...
    function mint(uint256 pack) public{
        ...
        _mint(msg.sender, nftIdsToMint, false);
    }
